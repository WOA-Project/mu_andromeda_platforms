/** @file
  ...
  Implements the UEFI Preloader Menu (Settings Menu).
...
#include "PreloaderMenuConfigAccess.h"
...
ListBox  *mTopMenu;
BOOLEAN  mShowFullMenu = FALSE;     // By default we won't show the full PreloaderMenu menu (requires validation if there's a system password).
...
BOOLEAN                          mTerminatePreloaderMenu = FALSE;
BOOLEAN                          mResetRequired;
FRONT_PAGE_AUTH_TOKEN_PROTOCOL   *mPreloaderMenuAuthTokenProtocol = NULL;
...
} mFormMap[] =
{
  ...
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------
  { 0, 0,            STRING_TOKEN (STR_MF_MENU_OP_PCINFO),    PRELOADER_MENU_CONFIG_FORMSET_GUID, PRELOADER_MENU_FORM_ID_PCINFO   },                           // PC info
  { 1, UNUSED_INDEX, STRING_TOKEN (STR_MF_MENU_OP_SECURITY),  PRELOADER_MENU_CONFIG_FORMSET_GUID, PRELOADER_MENU_FORM_ID_SECURITY },                           // Security
  ...
  { 5, 2,            STRING_TOKEN (STR_MF_MENU_OP_EXIT),      PRELOADER_MENU_CONFIG_FORMSET_GUID, PRELOADER_MENU_FORM_ID_EXIT     }                            // Exit
};

// PreloaderMenu form set GUID
//
EFI_GUID  gMuPreloaderMenuConfigFormSetGuid = PRELOADER_MENU_CONFIG_FORMSET_GUID;
...
PRELOADER_MENU_CALLBACK_DATA  mPreloaderMenuPrivate = {
  PRELOADER_MENU_CALLBACK_DATA_SIGNATURE,
  NULL,
  NULL,
  NULL,
  {
    ExtractConfig,        // Lives in PreloaderMenuConfigAccess.c
    RouteConfig,          // Lives in PreloaderMenuConfigAccess.c
    NULL
  }
};

HII_VENDOR_DEVICE_PATH  mPreloaderMenuHiiVendorDevicePath = {
  {
    {
    ...
    PRELOADER_MENU_CONFIG_FORMSET_GUID
  },
  {
  ...
  do {
    ...
    for (Index = 0; Index < FmpCount; Index++) {
      ...
      if (FmpImageInfoCount > 1) {
        DEBUG ((DEBUG_INFO, "%a - Found %d descriptors.  For PreloaderMenu we only show the 1st descriptor.\n", __FUNCTION__, FmpImageInfoCount));
      }
...
FmpCleanUp:
    ...
    Status = HiiUpdateForm (
               ...
               &gMuPreloaderMenuConfigFormSetGuid, // Formset GUID
               PRELOADER_MENU_FORM_ID_PCINFO,      // Form ID
  ...
  )
{
  ...
  if (InitializeHiiData) {
    ...
    Status = gBS->InstallMultipleProtocolInterfaces (
                    &mPreloaderMenuPrivate.DriverHandle,
                    &gEfiDevicePathProtocolGuid,
                    &mPreloaderMenuHiiVendorDevicePath,
                    &gEfiHiiConfigAccessProtocolGuid,
                    &mPreloaderMenuPrivate.ConfigAccess,
                    NULL
                    );
    ...
    //
    mPreloaderMenuPrivate.HiiHandle = HiiAddPackages (
                                    &gMuPreloaderMenuConfigFormSetGuid,
                                    mPreloaderMenuPrivate.DriverHandle,
                                    PreloaderMenuVfrBin,
                                    PreloaderMenuStrings,
                                    NULL
                                    );
    if (mPreloaderMenuPrivate.HiiHandle == NULL) {
      return EFI_OUT_OF_RESOURCES;
    }
  }

  HiiHandle = mPreloaderMenuPrivate.HiiHandle;
...
/**
  Uninitialize HII information for the PreloaderMenu
...
**/
EFI_STATUS
UninitializePreloaderMenu (
  VOID
  )
{
  EFI_STATUS  Status = EFI_SUCCESS;

  // Dispose the auth token we acquired for the Preloader Menu.
  ...
  if (NULL != mPreloaderMenuAuthTokenProtocol) {
    Status = gBS->UninstallMultipleProtocolInterfaces (
                    mImageHandle,
                    &gMsFrontPageAuthTokenProtocolGuid,
                    mPreloaderMenuAuthTokenProtocol,
                    NULL
                    );
    FreePool (mPreloaderMenuAuthTokenProtocol);
    mPreloaderMenuAuthTokenProtocol = NULL;
    ASSERT_EFI_ERROR (Status);
  }
  ...
  //
  HiiRemovePackages (mPreloaderMenuPrivate.HiiHandle);
  if (mPreloaderMenuPrivate.LanguageToken != NULL) {
    FreePool (mPreloaderMenuPrivate.LanguageToken);
    mPreloaderMenuPrivate.LanguageToken = (EFI_STRING_ID *)NULL;
  }
...
/**
  Call the browser and display the Preloader Menu
  ...
  )
{
  ...
  Handles[0]  = mPreloaderMenuPrivate.HiiHandle;
  HandleCount = 1;
  ...
  // If the user selected the "Restart now" button to exit the PreloaderMenu, set the exit flag.
  //
  if (ActionRequest == EFI_BROWSER_ACTION_REQUEST_EXIT) {
    mTerminatePreloaderMenu = TRUE;
  }
...
/**
  ...
  enabled, then enable the DFCI tab of OemPreloaderMenu.
  ...
  )
{
  ...
  for (Count = 0; Count < MenuOptionCount; Count++) {
    ...
    if ((UNUSED_INDEX != Index) && (Index < MenuOptionCount)) {
      MenuOptions[Index].CellText = HiiGetString (mPreloaderMenuPrivate.HiiHandle, mFormMap[Count].MenuString, NULL);
    }
  }
...
/**
  Draws the Preloader Menu Title Bar.
  ...
  )
{
  ...
  GetTextStringBitmapSize (
    HiiGetString (mPreloaderMenuPrivate.HiiHandle, STRING_TOKEN (STR_PRELOADER_MENU_TITLE), NULL),
    &StringInfo.FontInfo,
    FALSE,
...
/**
  Draws the PreloaderMenu Master Frame and its Top-Level menu.
  ...
  )
{
  ...
  //
  if (SecViolation) {
    DEBUG ((DEBUG_INFO, "PreloaderMenu::%a - SecureBoot violation detected! Warning user...\n", __FUNCTION__));
...
STATIC
EFI_STATUS
InitializePreloaderMenuUI (
  VOID
  )
{
  EFI_STATUS  Status = EFI_SUCCESS;

  // Establish initial PreloaderMenu TitleBar and Master Frame dimensions based on the current screen size.
  //
  ...
  )
{
  ...
  // Register Preloader Menu strings with the HII database.
  //
  InitializeStringSupport ();

  // Initialize HII data (ex: register strings, etc.).
  //
  InitializePreloaderMenu (TRUE);

  // Initialize the PreloaderMenu User Interface.
  //
  HandleSecureBootChange();

  // Initialize the FrontPage User Interface.
  //
  Status = InitializePreloaderMenuUI ();
  ...
  Status            = EFI_SUCCESS;
    ...
    CallPreloaderMenu (mCurrentFormIndex);
  ...
  )
{
  ...
  if (!EFI_ERROR (Status) && (mAuthToken != DFCI_AUTH_TOKEN_INVALID)) {
    mPreloaderMenuAuthTokenProtocol = (FRONT_PAGE_AUTH_TOKEN_PROTOCOL *)AllocateZeroPool (sizeof (mPreloaderMenuAuthTokenProtocol));

    //
    // Regardless of the auth token value we install the protocol.
    // When system password is set, if user enters a invalid password, then the PreloaderMenu access will be restricted.
    ...
    //
    mPreloaderMenuAuthTokenProtocol->AuthToken = (UINTN)mAuthToken;
    Status                                 = gBS->InstallMultipleProtocolInterfaces (
                                                    &mImageHandle,
                                                    &gMsFrontPageAuthTokenProtocolGuid,
                                                    mPreloaderMenuAuthTokenProtocol,
    ...
    if (Status == EFI_SUCCESS) {
      DEBUG ((DEBUG_INFO, "%a PreloaderMenuAuthTokenProtocol was successfully installed %r\n", __FUNCTION__, Status));
    }
  }
