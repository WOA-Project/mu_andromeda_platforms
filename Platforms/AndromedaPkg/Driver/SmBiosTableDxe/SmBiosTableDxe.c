...
CHAR8 *mEnclosureInfoType3Strings[] = {
    "Microsoft Corporation", UNKNOWN_STRING_NAME, UNKNOWN_STRING_NAME,
...
CHAR8 *mMemDevInfoType17Strings[] = {
    ...
    UNKNOWN_STRING_NAME,
    UNKNOWN_STRING_NAME,
    "H9HKNNNEBMAVAR-NEH", // H9HKNNNFBMAVAR-NEH for 8GB
    NULL};
    ...
    CHAR8 *RetailSkuString)
{
  ...
  GetUUIDFromEFIChipInfoSerialNumType(&mSysInfoType1.Uuid, sizeof(GUID));
    ...
    IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable)
{
  ...
  CHAR8  SerialNumberString[PLATFORM_TYPE_STRING_MAX_SIZE];
  ...
  Status = RetrieveSerialNumber(SerialNumberString);
  if (EFI_ERROR(Status)) {
    AsciiStrnCpyS(
        SerialNumberString, PLATFORM_TYPE_STRING_MAX_SIZE, UNKNOWN_STRING_NAME,
  ...
  if (AsciiStrLen(SmbiosSystemRetailModel) > 0) {
    ...
    Status = RetrieveChipVersion(VersionString);
    if (EFI_ERROR(Status)) {
      AsciiStrnCpyS(
  ...
  RetrieveSurfaceInformation(
      SerialNumberString, VersionString, RetailSkuString);

  BIOSInfoUpdateSmbiosType0();
  SysInfoUpdateSmbiosType1(
      ProductNameString, VersionString, SerialNumberString, RetailSkuString);
