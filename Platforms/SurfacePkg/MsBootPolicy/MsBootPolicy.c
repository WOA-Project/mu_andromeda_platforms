  ...
  )
{
  ...
  for (i = 0; i < HandleCount; i++) {
    ...
    if (Tmp != NULL) {
      DEBUG ((DEBUG_WARN, "%3d %s", i, Tmp));    // Output newline in different call as
    } else {
      DEBUG ((DEBUG_WARN, "%3d NULL\n", i));
    }
  ...
  )
{
  ...
  SwapCount = 0;
  DEBUG ((DEBUG_WARN, "SortHandles - Before sorting\n"));
  ...
  DEBUG ((DEBUG_WARN, "SortHandles - After sorting\n"));
  DisplayDevicePaths (HandleBuffer, HandleCount);
  DEBUG ((DEBUG_WARN, "Exit %a, SwapCount = %d\n", __FUNCTION__, SwapCount));
  return;
}
  ...
  )
{
  ...
  for (Index = 0; Index < HandleCount; Index++) {
    ...
    DEBUG ((DEBUG_WARN, "Selecting device %s", TmpStr));
    DEBUG ((DEBUG_WARN, "\n"));
    if (MsBootPolicyLibIsDeviceBootable (Handles[Index])) {
      ...
      if ((ByFilter == FilterOnlyIPv4) || (ByFilter == FilterOnlyIPv6) || (ByFilter == FilterOnlyUSB)) {
        DEBUG ((DEBUG_WARN, "Attempting alternate boot...\n"));
        Status = SetAltBoot ();
  ...
  for (OptionCount = 0; OptionCount < Count; OptionCount++) {
    ...
    if (Flag == FALSE) {
      Status = gRT->SetVariable (
                      &ArrayList[OptionCount * OptionLength],
                      &gEfiGlobalVariableGuid,
                      0,
                      0,
                      NULL
                      );
      DEBUG ((DEBUG_WARN, "%a Deleting the Unused load option %s\n", __FUNCTION__, &ArrayList[OptionCount * OptionLength]));
    }
  }
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "Wait for Event!\n"));
  ...
  )
{
  ...
  DEBUG ((DEBUG_WARN, "%a parameter = %a\n", __FUNCTION__, Parameters));
  ...
  Status = EFI_SUCCESS;
  DEBUG ((DEBUG_WARN, "%a starting with parm %c\n", __FUNCTION__, *Parameters));
  Index = 0;
  while (BootSequence[Index] != MsBootDone) {
    DEBUG ((DEBUG_WARN, "Attempting boot type %d\n", BootSequence[Index]));
...
Done:
  DEBUG ((DEBUG_WARN, "Exit boot policy. Index=%d, BS=%d, Status=%r\n", Index, BootSequence[Index], Status));
  return Status;
}
